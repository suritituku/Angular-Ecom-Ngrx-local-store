...
import { addProduct, clearCart, removeProduct, removeProductall } from './cart.actions';
...
export const cartReducer = createReducer(
  ...
  on(removeProductall, (entries, product) => {
    const entriesClone: Product[] = JSON.parse(JSON.stringify(entries));
    const found = entriesClone.find(e => e.id == product.id);
    ...
    if (found?.id) {
        entriesClone.splice(entriesClone.lastIndexOf(found), 2)
